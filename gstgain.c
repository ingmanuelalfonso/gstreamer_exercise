...
// Install to your plugin dir or set GST_PLUGIN_PATH.
...
struct _GstGainBP {
  ...
  // Biquad state per channel for HPF and LPF
  typedef struct {
    float b0, b1, b2, a1, a2;   // a0 normalized to 1
    float x1, x2, y1, y2;
  } Biquad;

  Biquad *hpf; // one per channel
  Biquad *lpf; // one per channel
};

G_DEFINE_TYPE (GstGainBP, gst_gainbp, GST_TYPE_AUDIO_FILTER)

// ------------------------ Utility: RBJ biquad design ------------------------
static void
biquad_reset (gpointer bqv, gint channels)
{
  // reset states to zero
  GstGainBP::Biquad *bq = (GstGainBP::Biquad *) bqv;
...
static void
design_lowpass (GstGainBP::Biquad *bq, double fs, double fc, double Q)
{
  if (fc <= 0.0 || fc >= fs * 0.49) {
    // bypass (b0=1, others 0)
...
static void
design_highpass (GstGainBP::Biquad *bq, double fs, double fc, double Q)
{
  if (fc <= 0.0 || fc >= fs * 0.49) {
    // bypass (b0=1, others 0) -> but for HPF, fc<=0 should bypass; fc>=Nyq should mute; keep bypass here
    bq->b0 = 1.0f; bq->b1 = 0.0f; bq->b2 = 0.0f; bq->a1 = 0.0f; bq->a2 = 0.0f;
    return;
  }
  double w0 = 2.0 * M_PI * fc / fs;
...
static inline float
biquad_process (GstGainBP::Biquad *s, float x)
{
  // Direct Form I
...
// ------------------------ Filter (re)design ------------------------
static void
(re)design_filters (GstGainBP *self)
{
  if (self->rate <= 0 || self->channels <= 0)
    return;

  if (!self->hpf) {
    self->hpf = g_new0 (GstGainBP::Biquad, self->channels);
  }
  if (!self->lpf) {
    self->lpf = g_new0 (GstGainBP::Biquad, self->channels);
  }

  const double fs = (double) self->rate;
  // Butterworth-ish: Q = 1/sqrt(2)
  ...
  if (low >= high && high > 0.0) {
    // Ensure band makes sense; if not, pull low just below high
    low = high * 0.8;
  }

  // Design a single set of coeffs then copy to all channels
  GstGainBP::Biquad hpf, lpf;
  ...
  GST_DEBUG_OBJECT (self, "Filters redesigned: fs=%d Hz, lowcut=%.2f Hz, highcut=%.2f Hz, ch=%d",
                    self->rate, low, high, self->channels);
}

// ------------------------ GstAudioFilter vfuncs ------------------------
static gboolean
gst_gainbp_setup (GstAudioFilter *filter, const GstAudioInfo *info)
{
  ...
  // Only accept F32LE here; audioconvert/audioresample handle the rest
  if (GST_AUDIO_INFO_FORMAT (info) != GST_AUDIO_FORMAT_F32LE) {
    GST_ERROR_OBJECT (self, "Unsupported format (expected F32LE)");
    return FALSE;
  }

  self->coeffs_valid = FALSE; // trigger redesign on first transform
  return TRUE;
}
...
gst_gainbp_transform_ip (GstBaseTransform *base, GstBuffer *buf)
{
  ...
  if (!self->coeffs_valid)
    (re)design_filters (self);

  GstMapInfo map;
  if (!gst_buffer_map (buf, &map, GST_MAP_READWRITE))
    return GST_FLOW_ERROR;

  // Map as audio frame for convenience
  GstAudioInfo *ainfo = &GST_AUDIO_FILTER (self)->info;
  gint channels = self->channels;
  gint rate = self->rate;
  (void)rate; // not used further

  // Interleaved float32
  gint n_samples_total = map.size / sizeof(float);
...
{
  // Just in case stop() wasn’t called
...
gst_gainbp_class_init (GstGainBPClass *klass)
{
  ...
  g_object_class_install_property (
      ...
      g_param_spec_double ("gain", "Gain",
                           "Linear gain factor (e.g., 1.0 = unity, 2.0 = +6dB)",
                           0.0, 64.0, 1.0,
  ...
  // Pad templates: F32LE, any rate/channels
  ...
  gst_audio_filter_class_add_pad_templates (af_class, caps, caps);
  gst_caps_unref (caps);
}
...
gst_gainbp_init (GstGainBP *self)
{
  ...
  // We want same caps in/out (F32LE)
